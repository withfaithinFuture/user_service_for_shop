...
def downgrade() -> None:
    ...
    op.create_table('market',
        ...
        sa.UniqueConstraint('marketName', name=op.f('market_marketName_key'), postgresql_include=[], postgresql_nulls_not_distinct=False)
    )
    ...
    op.create_table('products',
        ...
        sa.Column('category', postgresql.ENUM('electronics', 'clothing', 'food', 'home', 'beauty', 'sports', 'books', 'other', name='productcategory'), autoincrement=False, nullable=False),
        ...
        sa.Column('createdAt', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=False),
        sa.ForeignKeyConstraint(['marketId'], ['market.marketId'], name=op.f('products_marketId_fkey')),
    ...
    op.create_foreign_key(op.f('order_markets_marketId_fkey'), 'order_markets', 'market', ['marketId'], ['marketId'])
    op.create_foreign_key(op.f('cartItems_productId_fkey'), 'cartItems', 'products', ['productId'], ['id'])
    op.create_foreign_key(op.f('OrderItems_productId_fkey'), 'OrderItems', 'products', ['productId'], ['id'])