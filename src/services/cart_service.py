...
class CartService:
    ...
    async def get_products_from_seller(self, products_ids: list[UUID]) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                raise HTTPException(status_code=503, detail="Сервис товаров временно недоступен")
    ...
    async def update_cart_item_service(self, user_id: UUID, data: UpdateCartItemRequestSchema) -> dict:
        ...
        if data.quantity > prod_data['available']:
            raise NotEnoughStockError(product_id=data.productId, available=prod_data['available'], requested=data.quantity)

        cart_cnt = await self.cart_rep.update_items_quantity(user_id=user_id, product_id=data.productId, quantity=data.quantity)
    ...
    async def delete_cart_item_service(self, user_id: UUID, product_id: UUID) -> dict:
        cart_cnt = await self.cart_rep.delete_cart_item(user_id=user_id, product_id=product_id)
        cart_data = await self.get_cart_service(user_id)
